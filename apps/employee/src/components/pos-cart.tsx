...
export interface POSCartItem {
  ...
  notes?: string;
  promoId?: string; // ID da promoção a que pertence (undefined = item avulso)
  sizeName?: string; // Nome do tamanho da pizza (ex: "Grande")
  flavorNames?: string[]; // Nomes dos sabores da pizza
}

export interface AppliedPromo {
  id: string; // ID único desta aplicação de promo
  code: string;
  description: string;
  discount: number;
  bundlePrice?: number;
...
export function POSCart({
  ...
  // Separar itens por promoção e itens avulsos
            ...
            {promoGroups.map((group) =>
              group.items.length > 0 ? (
                ...
                >
                  ...
                  {/* Combo Items (compact, sem preço individual) */}
